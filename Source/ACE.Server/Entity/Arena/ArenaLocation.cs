...
namespace ACE.Server.Entity
{
    public class ArenaLocation
    {
                    ...
                    if (shouldReward)
                    {
                        switch (ActiveEvent.EventType)
                        {
                            ...
                            case "2v2":
                                ...
                                {
                                    //50% xp to next level
                                    player.GrantLevelProportionalXp(0.3, 0, 0, true);
                                }
                                ...
                                //Give 5 Nexus Tokens
                                var nexustoken = WorldObjectFactory.CreateNewWorldObject(1000002); //Nexus Token
                                nexustoken.SetStackSize(3);
                                var nexustokenCreateResult = player.TryCreateInInventoryWithNetworking(nexustoken);
                                if (nexustokenCreateResult)
                                {
                                    player.Session.Network.EnqueueSend(new GameMessageCreateObject(nexustoken));
                                    var msg = new GameMessageSystemChat($"You have received 3 Nexus Tokens", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }

                                //Give 1 Arena Token
                                var arenaTrophy = WorldObjectFactory.CreateNewWorldObject(1000003); //Arena Token
                                ...
                                if (arenaTrophyCreateResult)
                                {
                                    ...
                                    var msg = new GameMessageSystemChat($"You have received a Arena Token", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }

                                //Give 1 Rookie’s Bloodstained Keys
                                var arenaKey = WorldObjectFactory.CreateNewWorldObject(480608); //Rookie’s Bloodstained Keys
                                ...
                                if (arenaKeyCreateResult)
                                {
                                    ...
                                    var msg = new GameMessageSystemChat($"You have received one of Rookie’s Bloodstained Keys", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }                                

                                //Give 25% chance for extra 1 to 3 Rookie’s Bloodstained Keys
                                if (new Random().NextDouble() > 0.90)
                                {
                                    ...
                                    for (int i = 0; i < bonusCount; i++)
                                    {
                                        ...
                                        {
                                            player.Session.Network.EnqueueSend(new GameMessageCreateObject(arenaKey));
                                            var msg = new GameMessageSystemChat($"You have received a bonus Rookie’s Bloodstained Key", ChatMessageType.Broadcast);
                                            player.Session.Network.EnqueueSend(msg);
                                        }
                                    }
                                }
                            ...
                            case "ffa":
                                ...
                                if (player.MaximumLuminance != null)
                                {
                                    player.GrantLuminance(40000, XpType.Quest, ShareType.None);
                                }

                                //Give 5 Nexus Tokens
                                var ffaWinnernexustoken = WorldObjectFactory.CreateNewWorldObject(1000002); //Nexus Token
                                ffaWinnernexustoken.SetStackSize(3);
                                var ffaWinnernexustokenCreateResult = player.TryCreateInInventoryWithNetworking(ffaWinnernexustoken);
                                if (ffaWinnernexustokenCreateResult)
                                {
                                    player.Session.Network.EnqueueSend(new GameMessageCreateObject(ffaWinnernexustoken));
                                    var msg = new GameMessageSystemChat($"You have received 3 Nexus Tokens", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }

                                //Give 3 Arena Token
                                var ffaWinner_arenaTrophy = WorldObjectFactory.CreateNewWorldObject(1000003); //Arena Token
                                ffaWinner_arenaTrophy.SetStackSize(2);
                                ...
                                if (ffaWinner_arenaTrophyCreateResult)
                                {
                                    ...
                                    var msg = new GameMessageSystemChat($"You have received 2 Arena Tokens", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }

                                //Give 5 Rookie’s Bloodstained Keys
                                for (int i = 0; i < 5; i++)
                                {
                                    var ffaWinner_arenaKey = WorldObjectFactory.CreateNewWorldObject(480608); //Rookie’s Bloodstained Keys
                                    ...
                                    if (ffaWinner_arenaKeyCreateResult)
                                    {
                                        ...
                                        var msg = new GameMessageSystemChat($"You have received five of Rookie’s Bloodstained Keys", ChatMessageType.Broadcast);
                                        player.Session.Network.EnqueueSend(msg);
                                    }
                                }
                                break;
                            ...
                            case "group":
                                ...
                                //Give 5 Nexus Tokens, triple if you were still alive and not fighting against clanmates
                                var groupWinnernexustoken = WorldObjectFactory.CreateNewWorldObject(1000002); //Nexus Token
                                groupWinnernexustoken.SetStackSize(5 * rewardMultiplier);
                                var groupWinnernexustokenCreateResult = player.TryCreateInInventoryWithNetworking(groupWinnernexustoken);
                                if (groupWinnernexustokenCreateResult)
                                {
                                    player.Session.Network.EnqueueSend(new GameMessageCreateObject(groupWinnernexustoken));
                                    var msg = new GameMessageSystemChat($"You have received {5 * rewardMultiplier} Nexus Tokens", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }

                                //Give 1 Arena Token, triple if you were still alive and not fighting against clanmates
                                var groupWinner_arenaTrophy = WorldObjectFactory.CreateNewWorldObject(1000003); //Arena Token
                                ...
                                if (groupWinner_arenaTrophyCreateResult)
                                {
                                    ...
                                    var msg = new GameMessageSystemChat($"You have received {1 * rewardMultiplier} Arena Tokens", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }

                                //Give 2 Rookie’s Bloodstained Keys, triple if you were still alive and not fighting against clanmates
                                for (int i = 0; i < 2 * rewardMultiplier; i++)
                                {
                                    var groupWinner_arenaKey = WorldObjectFactory.CreateNewWorldObject(480608); //Rookie’s Bloodstained Keys
                    ...
                    if (shouldReward)
                    {
                        ...
                        switch (ActiveEvent.EventType)
                        {
                            ...
                            case "2v2":
                                ...
                                //Give 1 Nexus Token
                                var nexustoken = WorldObjectFactory.CreateNewWorldObject(1000002); //Nexus Token
                                nexustoken.SetStackSize(1);
                                var nexustokenCreateResult = player.TryCreateInInventoryWithNetworking(nexustoken);
                                if (nexustokenCreateResult)
                                {
                                    player.Session.Network.EnqueueSend(new GameMessageCreateObject(nexustoken));
                                    var msg = new GameMessageSystemChat($"You have received a Nexus Token", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }

                                //Give 25% chance for 1 Rookie’s Bloodstained Keys
                                if (new Random().NextDouble() > 0.90)
                                {
                                    var arenaKey = WorldObjectFactory.CreateNewWorldObject(480608); //Rookie’s Bloodstained Keys
                                    arenaKey.SetStackSize(1);
                            ...
                            case "ffa":
                                ...
                                if (loser.FinishPlace == 2)
                                {
                                    ...
                                    //Give 3 Nexus Tokens
                                    var ffaLosernexustoken = WorldObjectFactory.CreateNewWorldObject(1000002); //Nexus Token
                                    ffaLosernexustoken.SetStackSize(3);
                                    var ffaLosernexustokenCreateResult = player.TryCreateInInventoryWithNetworking(ffaLosernexustoken);
                                    if (ffaLosernexustokenCreateResult)
                                    {
                                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(ffaLosernexustoken));
                                        var msg = new GameMessageSystemChat($"You have received 3 Nexus Tokens", ChatMessageType.Broadcast);
                                        player.Session.Network.EnqueueSend(msg);
                                    }

                                    //Give 2 Arena Token
                                    var ffaLoser_arenaTrophy = WorldObjectFactory.CreateNewWorldObject(1000003); //Arena Token
                                    ...
                                    //Give 1 Rookie’s Bloodstained Keys
                                    var ffaLoser_arenaKey = WorldObjectFactory.CreateNewWorldObject(480608); //Rookie’s Bloodstained Keys
                                ...
                                else if (loser.FinishPlace == 3)
                                {
                                    ...
                                    //Give 1 Nexus Tokens
                                    var ffaLosernexustoken = WorldObjectFactory.CreateNewWorldObject(1000002); //Nexus Token
                                    ffaLosernexustoken.SetStackSize(1);
                                    var ffaLosernexustokenCreateResult = player.TryCreateInInventoryWithNetworking(ffaLosernexustoken);
                                    if (ffaLosernexustokenCreateResult)
                                    {
                                        player.Session.Network.EnqueueSend(new GameMessageCreateObject(ffaLosernexustoken));
                                ...
                                else
                                {
                                    ...
                                    //25% chance to give 1 Rookie’s Bloodstained Keys
                            ...
                            case "group":
                                ...
                                //Give 2 Nexus Tokens
                                var groupLosernexustoken = WorldObjectFactory.CreateNewWorldObject(1000002); //Nexus Token
                                groupLosernexustoken.SetStackSize(2);
                                var groupLosernexustokenCreateResult = player.TryCreateInInventoryWithNetworking(groupLosernexustoken);
                                if (groupLosernexustokenCreateResult)
                                {
                                    player.Session.Network.EnqueueSend(new GameMessageCreateObject(groupLosernexustoken));
                                    var msg = new GameMessageSystemChat($"You have received 2 Nexus Tokens", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }

                                //Give 1 Arena Token
                                var groupLoser_arenaTrophy = WorldObjectFactory.CreateNewWorldObject(1000003); //Arena Token
                                ...
                                if (groupLoser_arenaTrophyCreateResult)
                                {
                                    ...
                                    var msg = new GameMessageSystemChat($"You have received 1 Arena Token", ChatMessageType.Broadcast);
                                    player.Session.Network.EnqueueSend(msg);
                                }
                                ...
                                if (new Random().NextDouble() > 0.75)
                                {
                                    var groupLoser_arenaKey = WorldObjectFactory.CreateNewWorldObject(480608); //Rookie’s Bloodstained Keys
        ...
        public static Dictionary<uint, ArenaLocation> InitializeArenaLocations()
        {
            ...
            cow.ArenaName = "Gladiator's Arena";
            ...
            heptagon.ArenaName = "The Drowned Pit";
            locList.Add(heptagon.LandblockId, heptagon);

            //icebox
            var icebox = new ArenaLocation();
            icebox.LandblockId = 0x5950;
            icebox.SupportedEventTypes = new List<string>();
            icebox.SupportedEventTypes.Add("1v1");
    ...
    new List<Position>()
    {
        new Position(0x595002C2, 137.865723f, -159.649750f, 0.005000f, 0f, 0f, 0.014999f, 0.999888f),
        //0x595002C2 [137.865723 -159.649750 0.005000] 0.999888 0.000000 0.000000 0.014999
        new Position(0x595002BF, 142.448196f, -129.912125f, 0.005000f, 0f, 0f, 0.999978f, -0.006704f),
        //0x595002BF [142.448196 -129.912125 0.005000] -0.006704 0.000000 0.000000 0.999978
        new Position(0x595002C2, 141.860199f, -159.729660f, 0.005000f, 0f, 0f, -0.010000f, 0.999950f),
        //0x595002C2 [141.860199 -159.729660 0.005000] 0.999950 0.000000 0.000000 -0.010000
        new Position(0x595002BF, 135.567551f, -129.822784f, 0.005000f, 0f, 0f, 0.999978f, -0.006704f),
