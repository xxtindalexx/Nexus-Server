...
namespace ACE.Server.Command.Handlers
{
    public static class PlayerCommands
    {
        ...
        [CommandHandler("buyrename", AccessLevel.Player, CommandHandlerFlag.None, 1,
            "Purchase a character rename for 200 Nexus Tokens",
            "< New Name >")]
        public static void HandleBuyRename(Session session, params string[] parameters)
        {
            ...
            {
                CommandHandlerHelper.WriteOutputInfo(session, $"Renaming your character costs 200 Nexus Tokens. You don't have enough Nexus Tokens in your inventory to cover the cost.", ChatMessageType.Broadcast);
                return;
            }
                ...
                DatabaseManager.Shard.IsCharacterNameAvailable(newName, isAvailable =>
                {
                    ...
                    else
                    {
                        if (isFirstRenameUsed)
                        {
                            ...
                            {
                                CommandHandlerHelper.WriteOutputInfo(session, $"200 Nexus Tokens have been removed from your inventory", ChatMessageType.Broadcast);
                            }
                            else
                            {
                                CommandHandlerHelper.WriteOutputInfo(session, $"Error: failed consuming 200 Nexus Tokens from your inventory. Please try again or contact an admin for support.", ChatMessageType.Broadcast);

                                //Log this failure to the audit log
                                PlayerManager.BroadcastToAuditChannel(session.Player, $"Error: player {session.Player.Name} used /BuyRename command, and was verified to have enough Nexus Tokens, but failed to consume the Nexus Tokens with TryConsumeFromInventoryWithNetworking.");
                                return;
                            }                            
                        }                        
                    }
