...
namespace ACE.Server.WorldObjects
{
    partial class Player
    {
        ...
        public List<WorldObject> CalculateDeathItems(Corpse corpse)
        {
            ...
            //Nexus Token drop on death behavior
            if (IsPKDeath(corpse.KillerId))
            {
                ...
                if (LastnexustokenDropTime.HasValue &&
                    Last2nexustokenDropTime.HasValue &&
                    Last2nexustokenDropTime.Value > (Time.GetUnixTime() - 3600))
                {
                    shouldDropTrophy = false;
                }

                //Don't drop trophy if 10 have already dropped today
                if (nexustokenDropDay.HasValue && nexustokenDropsToday.HasValue)
                {
                    if (nexustokenDropDay.Value >= Time.GetUnixTime(DateTime.Today) &&
                        nexustokenDropsToday >= 10)
                    {
                        shouldDropTrophy = false;
                    }
                }
                ...
                if (shouldDropTrophy)
                {
                    ...
                    if (!LastnexustokenDropTime.HasValue)
                    {
                        SetProperty(PropertyFloat.LastnexustokenDropTime, Time.GetUnixTime());
                    }
                    else
                    {
                        SetProperty(PropertyFloat.Last2nexustokenDropTime, LastnexustokenDropTime.Value);
                        SetProperty(PropertyFloat.LastnexustokenDropTime, Time.GetUnixTime());
                    }

                    if (!nexustokenDropDay.HasValue ||
                        nexustokenDropDay < Time.GetUnixTime(DateTime.Today) ||
                        !nexustokenDropsToday.HasValue)
                    {
                        SetProperty(PropertyFloat.nexustokenDropDay, Time.GetUnixTime(DateTime.Today));
                        SetProperty(PropertyFloat.nexustokenDropsToday, 1);
                    }
                    else
                    {
                        SetProperty(PropertyFloat.nexustokenDropsToday, nexustokenDropsToday.Value + 1);
                    }
                }
                ...
                {
                    //For Town Control landblocks that always drop a set number of Nexus Tokens within a certain subset of cells
                    ...
                    if (townId.HasValue)
                    {
                        ...
                        {
                            var nexustoken = WorldObjectFactory.CreateNewWorldObject(1000002);
                            nexustoken.SetStackSize(1);
                            dropItems.Add(nexustoken);
